...
<template>
  <div class="hiring-request-quarter p-4">
    ...
    <div class="mb-6 space-y-4">
      ...
      <!-- Select Department -->
      <div class="filter">
        <label for="departmentSelect" class="block text-sm font-medium text-gray-700 mb-2">Select Department <span class="text-red-500">*</span></label>
        <select
          id="departmentSelect"
          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
    ...
    <div v-if="filteredRequests.length" class="mt-6">
      <h3 class="text-lg font-semibold text-gray-800 mb-4">Hiring Requests for {{ selectedQuarter }} in {{ selectedDepartment || 'All Departments' }}</h3>
      <table class="min-w-full bg-white border border-gray-300">
        <thead>
          <tr class="bg-gray-100">
            <th class="px-4 py-2 text-left text-sm font-medium text-gray-700">Job Title</th>
    ...
    <p v-else class="text-gray-500 mt-4">No hiring requests found for the selected quarter and department.</p>
...
const filteredRequests = computed(() => {
  return hiringRequests.value.filter(request => {
    const matchesQuarter = !selectedQuarter.value || request.quarter === selectedQuarter.value;
...
const filterRequests = () => {
  // No additional logic needed, computed property handles filtering
};
