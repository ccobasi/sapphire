...
const designationModalOption = ref('')
...
const tempHeadings = ["Designation","Department","Status","Action"]
...
onMounted(() => {
    store.currentpage = 'designations';
...
<template>
    <div class="vacancies mt-4">
        <div class="heading">
            <div class="title">
                <h1>Designations</h1>
                <p>Here are lists of your Designations</p>
            </div>
            <div class="buttons d-flex">
                <button class="btn btn-primary add-vacancy-button" @click="designationModalOption='add';showDepartmentModal=true">Add Designation</button>
                <button class="btn add-vacancy-button" @click="designationModalOption='import';showDepartmentModal=true">Import Designation</button>
            </div>
        </div>

        <div class="row">
            <TableOne :tableData=tempData :headings=tempHeadings :tableType="'designations'"  />
        </div>

        <div class="modal" v-show="showDepartmentModal">
            <div class="modal-content">
                <div class="top-section mt-4">
                    <h3 class="title text-capitalize">{{designationModalOption}} Designation</h3>
                ...
                <div class="subsection text-center mt-2">
                    <p v-if="designationModalOption==='add'">Input the designation name to get added</p>
                    <p v-if="designationModalOption==='import'">Download an excel template by clicking on the below button</p>
                    <small class="" v-if="designationModalOption==='import'">Note: please do not change the layout of the cells in the excel, you are expected to add your data below the column header (first row).</small>
                </div>
                

                <div class="main">
                    <div class="page d-flex align-items-center flex-column mt-4" v-if="designationModalOption==='import'">
                    ...
                    <div class="page flex-column" v-if="designationModalOption==='add'">
                   ...
                   <div class="save-button">
                        <button class="btn btn-primary" >{{designationModalOption==='add' ? 'add' : designationModalOption === 'import' ? 'upload' : ''}}</button>
                   </div>
                </div>
            </div>
        </div>
