<template>
  <div class="employee-dashboard">
    <div class="first">
      <div class="title">
        <h1>Probated Extension</h1>
        <p>Dashboard / Probated Extension</p>
      </div>
    </div>
    ...
    <table class="employee-table">
      <thead>
        <tr>
          ...
          <th @click="sort('extensionDate')">Extension Date</th>
          <th>Duration</th>
          <th>Action</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="employee in sortedEmployees" :key="employee.employeeCode">
          ...
          <td>{{ employee.extensionDate || 'N/A' }}</td>
          <td>{{ calculateDuration(employee.joinDate, employee.endDate) }}</td>
...
// Assume 'extensionDate' needs to be added to the employee data. 
// Here, we'll use 'joinDate' as 'extensionDate' for demonstration.
const employees = ref([
...
const viewDetails = (employee) => {
  ...
  alert(`Details for ${employee.name}:\nEmployee Code: ${employee.employeeCode}\nEmail: ${employee.email}\nExtension Date: ${employee.extensionDate || 'N/A'}\nDuration: ${calculateDuration(employee.joinDate, employee.endDate)}`);
};

onMounted(() => {
  store.currentpage = 'probated-extension';
